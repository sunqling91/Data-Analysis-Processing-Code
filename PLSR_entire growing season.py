# Analysis for the entire growing season using PCA and PLS
import warnings
from sklearn.cross_decomposition import PLSRegression
from sklearn.preprocessing import StandardScaler
from sklearn.decomposition import PCA  # Import PCA to calculate the explained variance ratio
...
# Load the data
file_path = 'C:/Users/lenovo/Desktop/土壤站点数据/11-14/数据-1/数据-1/逐物种结果提取/52765cq.xlsx'  # Change to your file path
...
time_periods = ['4-9']  # Use variables for the April-September period only
...
for plant_column in plant_columns:
    ...
    variables = [
        f'0-40cm{time_periods[0]}',
        f'pre{time_periods[0]}',
        f'TMEAN{time_periods[0]}',
        f'SSD{time_periods[0]}',
        gud_column
    ]
    ...
    if X.empty or len(y) < 2:
        continue  # If so, skip this combination
    ...
    # Calculate the explained variance ratio and cumulative explained variance ratio for each principal component
    ...
    # Build and fit the PLS (Partial Least Squares) regression model using the selected number of components
...
output_file_path = 'C:/Users/lenovo/Desktop/计算贡献25_1/5变量(TMEAN)/控制主成分的全期/52765cq.xlsx'  # Change to your desired output file path
